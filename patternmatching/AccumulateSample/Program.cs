...
//result = AccumulateRecursiveSpan(numbers);  // numbers.AsSpan with .NET 9
//Console.WriteLine($"Using generic {result}");
...
//    .WithOptions(ConfigOptions.DisableOptimizationsValidator); // disables only the validator, but avoids unsupported options
